...
- Corsia `HIGH_RISK`: inizialmente `STANDARD`, elevata quando il primo cold checkout ha rilevato un difetto cross-platform nel comando root `config:check`; test mirati, un solo `verify` finale, cold checkout richiesto dalla card e una self-review P0/P1.
...
- [x] **Step 1: eseguire i gate mirati sul functional head**
...
- [x] **Step 2: verificare che il progetto Compose non sia già in uso**
...
- [x] **Step 3: creare il worktree detached in un path verificato**
...
- [x] **Step 4: installare e avviare soltanto risorse locali sintetiche**
...
- [x] **Step 5: rimuovere il worktree senza attraversare path esterni**
...
- [x] **Step 1: registrare le evidenze locali senza creare un report separato**
...
- **Progresso:** `100%`
- **Esito test:** `PASSING`
- **Evidenze di chiusura:** contract 3/3; `verify:docs` exit 0; unico `verify` HIGH_RISK exit 0; clean checkout frozen install + PostgreSQL head + build + integration + `web-health-v1` exit 0; migration/eval/trace ID `000002_feature_flags` / `N/A` / `N/A`.
```
...
- [x] **Step 2: chiudere tracciabilità e contesto senza inventare un prossimo READY**
...
- [x] **Step 3: eseguire il gate finale sui metadata terminali**
...
Expected: il gate HIGH_RISK exit `0`; nessun whitespace error; solo file DOC-ARCH-001 intenzionali.

- [x] **Step 4: eseguire la self-review P0/P1**
...
- [x] **Step 5: committare il candidato finale**
...
```powershell
gh pr create --base main --head codex/doc-arch-001 --title "docs: complete architecture documentation foundation" --body "DOC-ARCH-001: ADR/overview, data model, local development and anti-drift contract. Targeted, full HIGH_RISK and clean cold-start gates pass. No Vercel action or deployment change."
```
