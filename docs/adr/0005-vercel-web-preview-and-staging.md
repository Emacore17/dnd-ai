...
Al checkpoint del 2026-07-14 esistono il progetto `dnd-ai-web` (`prj_lR2dL0wwAvLmDzjvbpDkhS3V7xoQ`) nello scope `emacore17s-projects` e il collegamento al repository autorizzato `Emacore17/dnd-ai` (repository ID `1299266814`). Root Directory `apps/web`, framework Next.js, regione `fra1`, Fork Protection, system environment variables, emissione OIDC e Trusted Source exact-match risultano configurati; le variabili applicative restano zero e non esiste alcun deployment. L'installation ID condivisa `41079282` vede 8 repository (`isAccessRestricted=false`), stato accettato e non bloccante. La branch GitHub `release/production` è stata creata da `ef803add249d16ded6f94936c59531047c8a92fa` e protetta dalla Ruleset dedicata `18926413`, attiva e senza bypass; la Ruleset `main-required-ci` `18877721` è invariata. L'environment GitHub `staging` resta limitato a `main`. Production Branch Vercel=`release/production`; il change set corrente registra atomicamente project ID, scope, origin `https://dnd-ai-web-git-main-emacore17s-projects.vercel.app` e installation ID, applica la deny-all con solo `main` abilitato e rende obbligatorio `deploy:check:linked`. L'origin non è ancora materializzata perché il merge non è avvenuto.

Prima dell'acceptance occorrono ancora: verificare zero deployment sulla PR; integrare il change set soltanto con `CI / Merge gate` verde; ottenere una Preview riuscita e confermare che il provider materializzi proprio l'origin versionata; provare negative deploy senza action `ready`, smoke o promozione; provare redeploy/revert. L'URL del dispatch non diventa per questo trusted: lo smoke continua a ignorarlo e usa soltanto l'origin versionata. Il readback deve inoltre confermare che project link, repository ID, Production Branch, Trusted Source, protezioni, environment GitHub, installation baseline condivisa e numero di deployment Production non siano cambiati. Se l'attivazione crea un deployment Production o richiede un upgrade/nuovi termini non autorizzati, fermarsi senza proseguire.
