...
- **Progresso:** `50%`
- **Esito test:** `PARTIAL`
- **Contesto verificato:** `YES` — checkpoint integrato su `main` a `70f726d5a7fd9feed1a338d4c24bbedecc0bbe0b`; account Hobby autorizzato verificato in forma redatta; project Vercel non-production collegato; Production Branch=`release/production` e zero deployment riletti; binding atomici e policy solo-`main` attivati nel branch `codex/bl-080-enable-preview`; spec SHA `26b3e86fdd4d0ef7835b2e9f5486820dbeac671c78d50de7a01c78471393fa1c`; data: `2026-07-14`
- **Priorità / stima:** `P0` / `M`
...
- **Evidenze di chiusura:** implementazione `50efcbe620ad7c1fc6eb3cf1b79cdb27b0c383af`; [PR #7](https://github.com/Emacore17/dnd-ai/pull/7), run PR [`29321410036`](https://github.com/Emacore17/dnd-ai/actions/runs/29321410036) e post-merge [`29321531038`](https://github.com/Emacore17/dnd-ai/actions/runs/29321531038) 5/5 `SUCCESS`; hardening commit `1766406b9bd701a9880705b371fdc0b05a73abe1`, [PR #10](https://github.com/Emacore17/dnd-ai/pull/10), [run `29326093430`](https://github.com/Emacore17/dnd-ai/actions/runs/29326093430) e merge `ef803add249d16ded6f94936c59531047c8a92fa`; checkpoint `main` `70f726d5a7fd9feed1a338d4c24bbedecc0bbe0b`; project `dnd-ai-web`/`prj_lR2dL0wwAvLmDzjvbpDkhS3V7xoQ`, scope `emacore17s-projects`, repository ID `1299266814`, GitHub App installation ID `41079282`, origin `https://dnd-ai-web-git-main-emacore17s-projects.vercel.app`, root `apps/web`, Next.js e `fra1`; Trusted Source OIDC exact-match; Production Branch Vercel=`release/production`; Ruleset release `18926413` e main `18877721` senza drift; zero env applicative e zero deploy; `deploy:check:linked`, contract e CI policy locali PASS; PR/smoke/failure/redeploy `pending`; report in `docs/testing/BL-080_VERIFICATION.md`
- **Note, rischi o bloccanti:** non include load, chaos, backup restore, production release o i gate UX della shell, posseduti da `BL-070`, `BL-079` e dai gate finali. Il piano Hobby personale/non-commerciale e l'identità esclusiva sono autorizzati; nessun upgrade, acquisto o account alternativo è consentito. GitHub `staging` resta protetto su `main`; il project Vercel è collegato a `Emacore17/dnd-ai` e la Trusted Source limita l'OIDC a audience, repository/repository ID, ref, environment e target `preview` attesi. Su istruzione esplicita del Product Owner l'installazione GitHub App condivisa `41079282` non viene ristretta: `isAccessRestricted=false` e gli 8 repository accessibili sono un rischio residuo accettato perché la restrizione toglierebbe accesso ad altri progetti; link repository/project esatto, Trusted Source exact-match, environment protetto, Ruleset senza bypass e policy branch-closed sono i controlli compensativi project-level. Il blocker Production Branch è chiuso; attivazione remota, deploy Preview, smoke, failure e redeploy restano pending. Un owner potrebbe ancora avviare un deploy/promote manuale, azione non autorizzata: ogni readback deve confermare zero Production. Il task resta `IN_PROGRESS` e `BL-079` resta `BACKLOG`.
...
git_branch: codex/bl-080-enable-preview
base_commit: 70f726d5a7fd9feed1a338d4c24bbedecc0bbe0b
current_commit: 70f726d5a7fd9feed1a338d4c24bbedecc0bbe0b
...
- **Commit/PR:** foundation `50efcbe620ad7c1fc6eb3cf1b79cdb27b0c383af`; [PR #7](https://github.com/Emacore17/dnd-ai/pull/7); hardening [PR #10](https://github.com/Emacore17/dnd-ai/pull/10); attivazione in corso su `codex/bl-080-enable-preview` da base `70f726d5a7fd9feed1a338d4c24bbedecc0bbe0b`
...
- **Exit code:** `0` per readback Production Branch/zero deployment, audit GitHub e full verify del change set in 65,3 s; CI/Preview restano da eseguire
...
- **Rischi residui/TODO tracciati:** account/piano autorizzati con vincolo Hobby esclusivo; installazione condivisa `41079282` invariata (`isAccessRestricted=false`, 8 repository) come rischio residuo accettato e compensato; PR/merge, negative deploy, smoke e redeploy reali ancora da provare; nessun deploy/promote manuale autorizzato; Trusted Source remota exact-match `PASS`; Production Branch Vercel=`release/production`; `release/production` protetta dalla Ruleset `18926413`, `main` dalla Ruleset invariata `18877721`; zero deploy
