...
- **Progresso:** `50%`
- **Esito test:** `PARTIAL`
- **Contesto verificato:** `YES` — foundation e hardening integrati su `main` a `ef803add249d16ded6f94936c59531047c8a92fa`; account Hobby autorizzato verificato in forma redatta; project Vercel non-production collegato ma auto-deploy ancora disabilitato; branch `release/production` creata e protetta; spec SHA `26b3e86fdd4d0ef7835b2e9f5486820dbeac671c78d50de7a01c78471393fa1c`; data: `2026-07-14`
- **Priorità / stima:** `P0` / `M`
...
- **Evidenze di chiusura:** implementazione `50efcbe620ad7c1fc6eb3cf1b79cdb27b0c383af`; [PR #7](https://github.com/Emacore17/dnd-ai/pull/7), run PR [`29321410036`](https://github.com/Emacore17/dnd-ai/actions/runs/29321410036) e post-merge [`29321531038`](https://github.com/Emacore17/dnd-ai/actions/runs/29321531038) 5/5 `SUCCESS`; evidenze living integrate fino alla CI post-merge `29322330969` 5/5 `SUCCESS`; hardening commit `1766406b9bd701a9880705b371fdc0b05a73abe1`, [PR #10](https://github.com/Emacore17/dnd-ai/pull/10), [run `29326093430`](https://github.com/Emacore17/dnd-ai/actions/runs/29326093430) e merge `ef803add249d16ded6f94936c59531047c8a92fa`; project `dnd-ai-web`/`prj_lR2dL0wwAvLmDzjvbpDkhS3V7xoQ`, scope `emacore17s-projects`, repository ID `1299266814`, GitHub App installation ID `41079282`, root `apps/web`, Next.js e `fra1` riletti dal provider; Trusted Source OIDC esatta configurata; `release/production` creata da `ef803add249d16ded6f94936c59531047c8a92fa` e protetta dalla Ruleset `release-production-required-ci` (`18926413`) con `CI / Merge gate` strict e `current_user_can_bypass=never`; Ruleset `main-required-ci` (`18877721`) invariata; zero env applicative e zero deploy; origin/smoke/redeploy `pending`; report in `docs/testing/BL-080_VERIFICATION.md`
- **Note, rischi o bloccanti:** non include load, chaos, backup restore, production release o i gate UX della shell, posseduti da `BL-070`, `BL-079` e dai gate finali. Il piano Hobby personale/non-commerciale e l'identità esclusiva sono autorizzati; nessun upgrade, acquisto o account alternativo è consentito. GitHub `staging` resta protetto su `main`; il project Vercel è collegato a `Emacore17/dnd-ai` e la Trusted Source limita l'OIDC a audience, repository/repository ID, ref, environment e target `preview` attesi. Su istruzione esplicita del Product Owner l'installazione GitHub App condivisa `41079282` non viene ristretta: `isAccessRestricted=false` e gli 8 repository accessibili sono un rischio residuo accettato perché la restrizione toglierebbe accesso ad altri progetti; link repository/project esatto, Trusted Source exact-match, environment protetto, Ruleset senza bypass e policy branch-closed sono i controlli compensativi project-level. L'unico blocker provider pre-attivazione è la Production Branch Vercel, riletta ancora come `main`, da impostare a `release/production`; origin, binding atomici, attivazione, deploy, smoke, failure e redeploy restano pending. Auto-deploy resta disabilitato, il task resta `IN_PROGRESS` e `BL-079` resta `BACKLOG`.
...
git_branch: codex/bl-080-activate-staging
base_commit: ef803add249d16ded6f94936c59531047c8a92fa
current_commit: ef803add249d16ded6f94936c59531047c8a92fa
...
- **Azioni esterne:** ambiente GitHub `staging` protetto creato nel repository pubblico e invariato. Il Product Owner ha autorizzato il solo piano Hobby personale/non-commerciale, la sola identità Vercel esclusiva verificata in forma redatta e GitHub `Emacore17`; restano vietati account alternativi, acquisti e upgrade. Creato e collegato il project non-production `dnd-ai-web` al repository target, senza deploy e senza env applicative. La Trusted Source GitHub Actions è configurata con claim esatti e installation ID `41079282` acquisito. Il Product Owner ha deciso esplicitamente di non restringere l'installazione condivisa (`isAccessRestricted=false`, 8 repository), perché la restrizione toglierebbe accesso ad altri progetti: il grant è rischio residuo accettato con controlli compensativi project-level. Il branch `release/production` è stato creato da `main` a `ef803add249d16ded6f94936c59531047c8a92fa` e protetto dalla Ruleset dedicata `release-production-required-ci` (`18926413`), strict su `CI / Merge gate` e senza bypass; `main-required-ci` (`18877721`) è invariata. Prima dell'attivazione resta obbligatorio impostare e rileggere in Vercel Production Branch=`release/production`, poi registrare atomicamente l'alias branch deterministico documentato con gli altri binding e provarne la materializzazione tramite deploy, smoke, failure e redeploy. Sono esclusi production, database/Redis/provider AI, comunicazioni pubbliche e qualunque secret in chat, log o file versionati.
...
- **Rischi/failure path:** token OIDC inviato a origin non trusted; credenziale provider esposta a codice PR non affidabile; installazione GitHub App condivisa con visibilità più ampia del singolo project, rischio accettato e compensato da binding/claim/policy project-level fail-closed; deploy non riconducibile al commit/App; regione o root directory in drift; URL payload stale o riutilizzato; errore provider mascherato; smoke che passa sulla pagina sbagliata; rollback che seleziona un artifact diverso; ambiente GitHub dichiarato ma non applicato.
...
- **Commit/PR:** implementazione `50efcbe620ad7c1fc6eb3cf1b79cdb27b0c383af`; [PR #7](https://github.com/Emacore17/dnd-ai/pull/7) integrata in `52bf58d9f9cb9cab6ad0cc1b1602d7556067b578`
- **Comandi eseguiti:** preflight Git/GitHub/Vercel; web lint/typecheck/build; unit/integration/contract/security; full `pnpm verify`; deployment e CI policy; API GitHub environment/branch/Ruleset; Vercel CLI/API per identity/plan/project/link/settings/env/deployment/Trusted Sources/Git namespace e repository grant readback; tentativi browser e Windows UI per Production Branch
- **Exit code:** `0` per full verify corrente in 70,8 s, gate locali finali, identity/plan/project/link/readback Vercel, Trusted Source, integration grant, creazione branch e Ruleset readback; un primo full verify post-review è scaduto con `124`/processi host residui, poi typecheck isolato e full verify forzato sono passati; `deploy:check:linked` resta intenzionalmente non-zero finché i quattro binding non possono essere registrati insieme; lettura GitHub `/user/installations` `403` attesa per il token OAuth `gh`; configurazione dashboard non eseguita per gli errori `Cannot redefine property: process` e `GetCursorPos failed: Accesso negato. (0x80070005)`, senza modifiche provider parziali
- **Report/CI URL o path:** `docs/testing/BL-080_VERIFICATION.md`; [CI PR `29321410036`](https://github.com/Emacore17/dnd-ai/actions/runs/29321410036); [CI post-merge `29321531038`](https://github.com/Emacore17/dnd-ai/actions/runs/29321531038); [hardening PR #10 / run `29326093430`](https://github.com/Emacore17/dnd-ai/actions/runs/29326093430); deploy/smoke URL `—`
- **Migration head:** `N/A`
...
- **Rischi residui/TODO tracciati:** account/piano autorizzati con vincolo Hobby esclusivo; Production Branch provider ancora `main`, unico blocker provider pre-attivazione; installazione condivisa `41079282` invariata (`isAccessRestricted=false`, 8 repository) come rischio residuo accettato dal Product Owner e compensato da controlli project-level; origin/binding, attivazione, negative deploy, smoke e redeploy reali ancora da provare; Trusted Source remota exact-match `PASS`; `release/production` protetta dalla Ruleset `18926413`, `main` dalla Ruleset invariata `18877721`; zero deploy
