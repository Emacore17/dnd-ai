...
- **Progresso:** `50%`
- **Esito test:** `FAILING`
- **Contesto verificato:** `YES` — [PR #12](https://github.com/Emacore17/dnd-ai/pull/12) integrata su `main` a `c64d09528dae2c1fd5e4ba3de7d17d15573dd71a` dopo CI 5/5 verde e zero deploy sulla PR; il primo deploy di `main` è però risultato `target=production` nonostante il readback Production Branch=`release/production`. Il deployment ha ricevuto alias prima della rimozione, poi deployment e alias del progetto sono tornati vuoti; hotfix fail-closed in corso su `codex/bl-080-fail-closed-hotfix`; spec SHA `26b3e86fdd4d0ef7835b2e9f5486820dbeac671c78d50de7a01c78471393fa1c`; data: `2026-07-14`
- **Priorità / stima:** `P0` / `M`
...
git_branch: codex/bl-080-fail-closed-hotfix
base_commit: c64d09528dae2c1fd5e4ba3de7d17d15573dd71a
current_commit: c64d09528dae2c1fd5e4ba3de7d17d15573dd71a
...
- **Azioni esterne:** ambiente GitHub `staging`, Ruleset e Trusted Source restano invariati; account/piano e grant condiviso mantengono i vincoli già registrati. La PR #12 ha attivato il desired state dopo CI verde e zero deploy sulla PR, ma il merge `c64d09528dae2c1fd5e4ba3de7d17d15573dd71a` ha creato un deployment `target=production` pur con readback Production Branch=`release/production`. La rimozione è stata richiesta mentre lo stato osservato era `BUILDING`; l'activity log prova che gli alias sono stati assegnati prima che la rimozione terminasse. Il deployment è ora assente e non restano alias del progetto; il dispatch `ready` ha generato un job smoke `skipped`. Il hotfix versionato ripristina fail-closed prima di ogni ulteriore indagine. Sono esclusi nuovi deploy, production, database/Redis/provider AI, comunicazioni pubbliche e qualunque secret in chat, log o file versionati.
...
- **Commit/PR:** foundation PR #7; hardening PR #10; attivazione commit `7335053`, PR #12 e merge `c64d095`; contenimento in corso su `codex/bl-080-fail-closed-hotfix`
...
- **Exit code:** `0` per CI PR #12, rimozione deployment, readback finale vuoto e full verify hotfix in 61,0 s; il gate remoto Preview è `FAILING` perché il target effettivo era Production
...
- **Rischi residui/TODO tracciati:** target provider inatteso ancora senza causa; account/piano autorizzati con vincolo Hobby esclusivo; installazione condivisa invariata come rischio accettato; nessun nuovo deploy/promote autorizzato; hotfix fail-closed, Preview/smoke/failure/redeploy e prova preventiva del target ancora aperti; lista deployment provider tornata a zero
