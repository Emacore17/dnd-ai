...
| Contenuto verificato | working tree del branch; commit finale e clean-checkout ancora pendenti |
...
L'implementazione e tutti i gate automatici specifici di BL-079 sono verdi. Il task non è `DONE`: restano i gate umani o ambientali richiesti dalla card — screen reader reale, telefono reale, review di comprensione con cinque utenti interni, browser manuali non Chromium, zoom 200%, staging posseduto da `BL-080` e verifica da commit pulito.
...
| `node --test tests/contracts/bl079-ui-foundation.test.mjs` | exit `0`; `8/8` |
...
Il performance smoke persistente azzera gli observer dopo font/load e misura scroll-to-latest, apertura/chiusura drawer e submit fixture. Il run corrente registra `0` long task e CLS `0.0000245`, sotto il guardrail `0.1`. Il caricamento iniziale e i device reali saranno rivalidati nei gate successivi.
