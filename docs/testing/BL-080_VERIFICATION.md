...
Finché questi punti non sono provati, `BL-080` resta `BLOCKED/50%/PARTIAL` e nessuna evidenza locale viene presentata come staging disponibile. Dopo l'integrazione di QA-002 non esiste alcun P0 `READY`: `GATE-M0` resta `BACKLOG` perché dipende dallo smoke remoto di `BL-080`.
