...
`BL-080` è `IN_PROGRESS/50%/FAILING`. [PR #12](https://github.com/Emacore17/dnd-ai/pull/12) ha superato tutti i gate senza deployment sulla PR, ma il merge `c64d09528dae2c1fd5e4ba3de7d17d15573dd71a` su `main` ha generato un deployment Vercel `target=production` nonostante il readback Production Branch=`release/production`. GitHub Deployment API lo registra come environment `Production` e status `success`; il job `Staging / Smoke` è stato rifiutato dal predicate Preview ed è `skipped`. Il deployment è stato eliminato e URL/alias ora rispondono `404`; deployment e alias del progetto sono tornati a zero. Il contenimento commit `4d3d4baad1a57b5340c0092209cc640499aa4da8`, [PR #13](https://github.com/Emacore17/dnd-ai/pull/13) e merge `61e5cbd2c3c1c258769fef6b3ad89853d7b7ca61` ha ripristinato la configurazione versionata disabilitata senza nuovi deploy. Il change set corrente aggiunge un guard build Preview-only, ma nessuno staging esiste ancora, la causa provider resta sconosciuta e ADR-0005 resta `proposed`.

| Campo | Valore |
|---|---|
| Data | 2026-07-14 |
| Ambiente locale | Windows; Node `24.11.0`; pnpm `10.34.5` |
| Branch di implementazione | `codex/bl-080-explicit-preview-guard` |
| Base verificata | `61e5cbd2c3c1c258769fef6b3ad89853d7b7ca61`; merge di contenimento PR #13 su `main` |
...
| `TURBO_FORCE=true corepack pnpm@10.34.5 verify` sul guard | PASS in 60,2 s; zero cache Turbo; unit 33 pass/1 skip host, security 14 pass/3 skip host, integration/contract/policy/scan/artifact verdi |
...
- guard Preview-only integrato su `main`, CI verde, readback system env/zero deploy e bootstrap CLI diagnostico con inspect immediato;
