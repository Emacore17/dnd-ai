...
`BL-080` aggiunge un workflow separato `Staging smoke`; non modifica il trust boundary della PR e non distribuisce da GitHub Actions. La PR #12 è rimasta senza deployment, ma il merge su `main` ha generato un deployment Production. Il relativo `repository_dispatch` ha creato un job `Staging / Smoke` `skipped`, perché il predicate accetta soltanto payload Preview validi. Il contenimento PR #13, il guard Preview-only PR #14 e la policy payload PR #15 sono integrati; le CI PR/post-merge della PR #15 (`29339984834`, `29340214947`) sono 5/5 verdi. Git auto-deploy resta disabilitato e CI verde non viene trattata come prova del target provider.
...
Il guard non seleziona l'environment e non impedisce al provider di creare un record deployment: può soltanto bloccare il completamento della build dopo che Vercel ha scelto il target. Il primo comando reale si era fermato sul limite file; dopo l'integrazione della policy payload, dry-run valido e `main` pulita, il singolo retry con `--target=preview --no-wait` ha creato comunque un record `target=production`. Il record è stato osservato `ERROR`, rimosso per ID esatto e l'origin restituisce `404`; activity log Vercel attribuisce il target Production alla CLI sul commit `1060228`. Nessun nuovo smoke è partito.
