...
- Fase corrente: repository collegato e Production Branch riletta `release/production`, ma il primo merge con policy solo-`main` ha creato un deployment `production`, poi eliminato. Il contenimento è integrato su `main` tramite PR #13: `source.autoDeploy=false`, binding versionati `null`, `git.deploymentEnabled=false` e Quality gate `deploy:check`; CI PR/post-merge sono verdi e il readback successivo conferma zero deployment. Il change set corrente aggiunge un guard build Preview-only senza riattivare Git. Il grant condiviso resta invariato per decisione PO.
...
| Build | Root Directory `apps/web`; framework Next.js; regione `fra1`; il nuovo `buildCommand` Preview-only è ancora da integrare e verificare remotamente |
...
7. Conservare il contenimento integrato: `git.deploymentEnabled=false`, `source.autoDeploy=false`, binding `null` e `deploy:check`. Integrare il guard Preview-only tramite PR con CI verde, poi rileggere `autoExposeSystemEnvs=true` e confermare nuovamente zero deployment/alias. Il provider build deve eseguire `node scripts/assert-vercel-preview-build.mjs` senza `--allow-local` prima del normale build.
...
Soltanto dopo merge del guard, CI verde, readback `autoExposeSystemEnvs=true` e conferma zero deployment, eseguire dalla root del monorepo il bootstrap diagnostico esplicito. Non usare `--cwd apps/web`: la Root Directory `apps/web` è già configurata sul progetto. Non usare `--prebuilt`, perché il guard deve essere eseguito nel build remoto con i metadata di sistema effettivi.
...
corepack pnpm dlx vercel@55.0.0 deploy . `
  --project dnd-ai-web `
...
corepack pnpm dlx vercel@55.0.0 inspect <deployment-id-or-url> `
  --scope emacore17s-projects `
  --format=json
```

Le tre asserzioni Git devono passare prima di creare qualunque record provider: il bootstrap usa esattamente la `main` remota integrata, senza file locali o SHA divergenti. L'inspect immediato deve poi mostrare il target Preview. Se il target è diverso, rimuovere subito soltanto quel deployment tramite ID esatto, confermare zero alias/deployment correnti e non attendere, promuovere o ritentare:
