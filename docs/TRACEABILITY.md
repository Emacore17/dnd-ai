...
Il repository pubblico è versionato e collegato a `Emacore17/dnd-ai`. `BL-001` ha introdotto lo scaffold applicativo, `BL-002` pipeline/Ruleset e `BL-003` config/startup fail-fast. `BL-004`, `BL-005`, `BL-008`, `BL-009`, `BL-010`, `BL-079`, `GOV-002`, `GOV-004`, `QA-001` e `DOC-ARCH-001` sono `DONE/100%/PASSING` e integrati su `main`. BL-005 è verificato tramite PR #28, candidate corretto `c2e4332b408f1cac3e2c69920cd18e5e772e87bf`, merge `e173fd9424ad77330ae8302f68affd4832d66798` e CI PR/post-merge `29525777416`/`29526030389`, entrambe 5/5 `SUCCESS`. `BL-006` è `IN_PROGRESS/50%/PARTIAL`: design `identity-access-v1`, artifact `v3`, config/domain, primitive crypto, migration `000004`, store e API access/reset sono implementati sulla branch; worker/BFF/UI e gate terminali restano aperti. `BL-081` resta `READY`, `QA-002` `BACKLOG` e `BL-080` congelato/bloccato.
...
| Login, session lifecycle, logout, revoca globale e reset one-time | spec §§20, 22.2, 22.8–22.12, 26.5, 26.8, 26.9 e 32 AC-01; ADR-0010; `identity-access-v1` | BL-006, QA-002 | contract artifact `v3`, porte/config/crypto, migration `000004_identity_access`, access store, service e sei route Fastify implementati; BFF e pagine `sign-in`, `reset-password`, `account/security` restano target | contract/config/crypto/startup e migration `PASS`; store PostgreSQL 2/2 + regressione 3/3 + security 2/2; service/cookie/API/security 20/20 e regressioni 23/23 provano rate pre-hash, hash/HMAC dummy, Origin, fixation, rotation, clear cookie e failure generici. Restano worker/BFF/UI, verticale completo e browser 320/390/1440 | persistence e API slice TDD verdi; runtime email/web e gate terminali aperti |
