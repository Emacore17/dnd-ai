...
test.describe("BL-079 performance smoke", () => {
  test("keeps core mobile interactions free of long tasks and disruptive layout shift", async ({
    page,
  }, testInfo) => {
    test.skip(
      testInfo.project.name !== "mobile-390",
    ...
    await page.addInitScript(() => {
      ...
      const state = {
        ...
        supportsLongTask: supportedEntryTypes.includes("longtask"),
      };

      performanceWindow.__bl079Performance = state;
    ...
    await page.evaluate(() => {
      const performanceWindow = window as typeof window & {
        __bl079Performance?: { cls: number; longTasks: number[] };
      };
      const state = performanceWindow.__bl079Performance;
    ...
    const metrics = await page.evaluate(() => {
      ...
      return performanceWindow.__bl079Performance;
    });

    expect(metrics, "performance observers must be initialized").toBeDefined();
    expect(metrics?.supportsLongTask).toBe(true);
    expect(metrics?.supportsLayoutShift).toBe(true);
    expect(
      metrics?.longTasks,
      "no interaction may block the main thread",
    ).toEqual([]);
